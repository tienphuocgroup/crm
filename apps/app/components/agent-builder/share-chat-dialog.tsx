...
export function ShareChatDialog({
	...
	const revoke = useMutation(
		trpc.conversations.revokeShare.mutationOptions({
			onSuccess: async (_result, input) => {
				...
				await invalidate();
				toast.success(t("chatLinkRevokedToast"));
			},
			onError: (error) => toast.error(error.message),
		}),
	);
	...
	const copyLink = async () => {
		...
		toast.success(t("chatLinkCopiedToast"));
	};
					...
					{status.isPending ? (
						...
						<>
							...
							<ShareChoice
								...
								label={t("shareAnyoneWithLink")}
								detail={t("readOnlyLabel")}
								disabled={createAction.pending || revokeAction.pending}
								onSelect={() => {
								...
								<div className="flex h-9 items-center gap-3 rounded-md border bg-background py-[3px] pr-[3px] pl-4">
									...
									>
										<AsyncButtonContent
											...
											pendingLabel={
												shareToken ? t("copyingLabel") : t("creatingLinkLabel")
											}
											successLabel={
												shareToken ? t("copiedLabel") : t("linkCreatedLabel")
											}
				...
				<DialogFooter className="flex-row items-center justify-between border-t p-4 px-5">
					...
					>
						<AsyncButtonContent
							...
							pendingLabel={t("revokingLabel")}
							successLabel={t("revokedLabel")}
							errorLabel={common("tryAgain")}
						>
							{t("revokeLinkButton")}
						</AsyncButtonContent>
