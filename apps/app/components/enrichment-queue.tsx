			...
			<PopoverTrigger asChild>
				<Button variant="outline" size="sm">
					<IndicatorDot
						...
						aria-hidden="true"
					/>
					{t("enrichmentQueue.trigger", { count: total })}
				</Button>
			</PopoverTrigger>
				...
				{visible.length === 0 ? (
					<div className="flex flex-col gap-1 px-6 py-8 text-center">
						<span className="font-medium text-popover-foreground text-sm">
							{t("enrichmentQueue.emptyTitle")}
						</span>
						<span className="text-muted-foreground text-xs">
							{t("enrichmentQueue.emptyDescription")}
						</span>
					</div>
				) : (
					<QueueRows
				...
				{footer.more > 0 ? (
					<div className="flex items-center justify-between px-4 py-3 text-xs">
						<span className="text-muted-foreground">
							{t("enrichmentQueue.moreQueued", { count: footer.more })}
						</span>
	...
	return (
		<Collapsible className="border-t" open={shown} onOpenChange={setShown}>
			<CollapsibleTrigger className="flex w-full cursor-pointer items-center justify-between px-4 py-3 text-xs hover:bg-muted">
				<span className="text-muted-foreground">
					{t("enrichmentQueue.scheduled", { count: total })}
				</span>
				<span className="text-primary">
					{shown ? t("enrichmentQueue.hide") : t("enrichmentQueue.show")}
				</span>
			</CollapsibleTrigger>
			...
			<CollapsibleContent className="max-h-60 overflow-y-auto border-t">
				...
				{total > rows.length ? (
					<div className="px-4 py-3 text-muted-foreground text-xs">
						{t("enrichmentQueue.scheduledMore", {
